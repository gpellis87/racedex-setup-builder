...
export default function ChatInterface({
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {messages.length === 0 && (
          <div className="animate-fade-in">
            <div className="text-center py-8">
              <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-orange-500/20 to-orange-700/20 border border-orange-500/30 flex items-center justify-center">
                <svg
                  className="w-8 h-8 text-racing-accent"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={1.5}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-lg mx-auto">
                  {QUICK_PROMPTS.map((prompt) => (
                    <button
                      ...
                      className="text-left px-3 py-2 rounded-lg border border-racing-border bg-racing-card hover:border-zinc-600 hover:bg-racing-surface transition-all text-xs text-zinc-400 hover:text-zinc-200"
                    >
                      &ldquo;{prompt}&rdquo;
                    </button>
                  ))}
      ...
      {/* Input */}
      <div className="border-t border-racing-border bg-racing-dark/80 backdrop-blur-md px-4 py-3">
        <form onSubmit={handleSubmit} className="flex gap-2 items-end">
          <div className="flex-1 relative">
            <textarea
              ...
              placeholder={
                hasCarAndTrack
                  ? "Describe how the car feels... (e.g. 'tight on entry, loose on exit')"
                  : "Select a car and track first..."
              }
              disabled={!hasCarAndTrack || isLoading}
              rows={1}
              className="w-full bg-racing-card border border-racing-border rounded-xl px-4 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-racing-accent/50 focus:ring-1 focus:ring-racing-accent/20 resize-none disabled:opacity-50 disabled:cursor-not-allowed"
            />
          </div>
          <button
            type="submit"
            disabled={!input.trim() || isLoading || !hasCarAndTrack}
            className="px-4 py-2.5 rounded-xl bg-racing-accent text-white font-semibold text-sm hover:bg-orange-600 transition-colors disabled:opacity-30 disabled:cursor-not-allowed shrink-0"
          >
            Send
          </button>
        </form>
