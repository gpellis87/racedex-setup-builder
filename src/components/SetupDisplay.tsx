  ...
  return (
    <div className="space-y-3 sm:space-y-4">
      ...
      {hasChanges && changedCount > 0 && (
        <div className="bg-gradient-to-br from-orange-500/10 to-orange-700/10 border border-racing-accent/30 rounded-xl p-3 sm:p-4 space-y-3">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-racing-accent/20 flex items-center justify-center shrink-0">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 py-3 sm:py-2.5 rounded-lg bg-racing-accent text-white font-bold text-xs hover:bg-orange-600 active:bg-orange-700 transition-colors flex items-center justify-center gap-1.5"
            >
            ...
            <button
              ...
              className="flex-1 py-3 sm:py-2.5 rounded-lg border border-racing-accent/40 text-racing-accent font-bold text-xs hover:bg-racing-accent/10 active:bg-racing-accent/20 transition-colors flex items-center justify-center gap-1.5"
            >
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {!hasChanges && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={onDownloadText}
              className="min-h-[36px] px-3 py-1.5 text-xs font-medium rounded-md bg-racing-card border border-racing-border hover:border-zinc-600 active:bg-racing-surface transition-colors"
            >
              .txt
            </button>
            <button
              onClick={onDownloadJSON}
              className="min-h-[36px] px-3 py-1.5 text-xs font-medium rounded-md bg-racing-accent text-white hover:bg-orange-600 active:bg-orange-700 transition-colors"
            >
              .json
            </button>
          </div>
      ...
      <div className="bg-racing-card border border-racing-border rounded-xl overflow-hidden">
        <div className="px-3 sm:px-4 py-2.5 sm:py-3 border-b border-racing-border bg-racing-surface">
        ...
        <div className="divide-y divide-racing-border">
          {car.parameters.map((group) => (
            <div key={group.group} className="px-3 sm:px-4 py-2.5 sm:py-3">
              ...
              <div className="space-y-1.5 sm:space-y-1">
                {group.params.map((param) => {
                  ...
                  return (
                    <div
                      key={param.key}
                      className={`flex items-baseline sm:items-center justify-between text-xs rounded-md px-1.5 py-1 sm:py-0.5 -mx-1.5 gap-2 ${
                        changed ? "bg-racing-accent/5" : ""
                      }`}
                    >
                      <span className={`${changed ? "text-zinc-200 font-medium" : "text-zinc-400"} shrink min-w-0`}>
                        {param.label}
                      </span>
                      <span className="font-mono font-medium flex items-center gap-1 sm:gap-1.5 shrink-0 text-right">
                        {changed && (
                          <span className="text-zinc-600 line-through text-[10px] hidden sm:inline">
                            {formatParamValue(oldVal, param.unit)}
                          </span>
                        )}
                        <span className={changed ? "text-racing-accent" : "text-zinc-200"}>
                          {formatParamValue(val, param.unit)} <span className="text-zinc-500">{param.unit}</span>
                        </span>
                        {changed && (
